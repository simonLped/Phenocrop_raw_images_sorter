...
### testzone
cal_pics_str = input("""input calibration pics after first flight (normally 1), then after next flight and so on (except)
      for last calibration pics in SDcard. (example: '1,1' for three fields)""")
cal_pics = cal_pics_str.split(',')
...
if confirm == "y" or confirm == "c":
    ...
    ################################### sorter part 1
    path = "D:/DCIM/"
    original_dir = sorted(os.listdir(path))
    
    x = 0
    y = 1
    for folder in original_dir:
        updated_dir = sorted(os.listdir(path))
        ...
        if folder.endswith('MEDIA'): 
            ...
            if new_folder_name in updated_dir:
                ...
                if x>0:
                    k = 0
                    folder_files = sorted(os.listdir('{}/{}'.format(path,folder)))
    ...
    new_folder_name = fields[x] + '_cal'
    last_folder_files = sorted(os.listdir('{}/{}'.format(path,original_dir[-1])))
    ...
    for folder in directory:
        ...
        # those files moved to the same folder name with no number
            ...
            directory_folder = sorted(os.listdir('{}/{}'.format(path,folder)))
